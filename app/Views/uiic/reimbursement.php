...
<script>
    function makeFieldsReadonly() {
        ...
        document.getElementById("emp_initial").readOnly = true;
        document.getElementById("emp_dob").readOnly = true;
        document.getElementById("emp_doj").readOnly = true;
    }
    function removeReadonly() {
        document.getElementById("emp_name").readOnly = false;
        document.getElementById("emp_initial").readOnly = false;
        document.getElementById("emp_dob").readOnly = false;
        document.getElementById("emp_doj").readOnly = false;
    }

    function handleEmpNumberChange() {
        let empNumber = document.getElementById("emp_id").value.trim();
        if (empNumber !== "") {
            $.ajax({
                url: '<?= base_url('uiic/get-user'); ?>',
                type: 'POST',
                dataType: 'json',
                data: {
                    id: empNumber
                ...
                success: function(response) {
                    $('#emp_name').val(response[0]['emp_name'].substring(0, response[0]['emp_name'].length - 1));
                    $('#emp_initial').val(response[0]['emp_name'].charAt(response[0]['emp_name'].length - 1));
                    $('#emp_dob').val(response[0]['dob']);
                    $('#emp_doj').val(response[0]['jdate']);
                },
                error: function(xhr, status, error) {
                    console.error(error);
                }
    ...
    document.getElementById("emp_id").addEventListener('input', handleEmpNumberChange);
    function handleRadioButtonChange() {
        let selectedValue = document.querySelector('input[name="employee_type"]:checked').value;
    ...
    let radios = document.querySelectorAll('input[name="employee_type"]');
